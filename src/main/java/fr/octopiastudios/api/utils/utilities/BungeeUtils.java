...
public class BungeeUtils {
    ...
    private static final WeakHashMap<Plugin, BungeeUtils> registeredInstances = new WeakHashMap<>();
    ...
    public static BungeeUtils get() {
        return registeredInstances.compute(plugin, (k, v) -> {
            if (v == null) v = new BungeeUtils(plugin);
            return v;
        });
    }
     ...
     *
     * @param serverName the server name of the server to get the player count of, or
     *                   ALL to get the global player count
    ...
    /**
     * Get a list of players connected on a certain server, or on ALL the
     * servers.
     *
     * @param serverName the name of the server to get the list of connected players,
     *                   or ALL for global online player list
     * @return A {@link CompletableFuture} that, when completed, will return a
     * list of players connected on a certain server, or on ALL the
     * servers.
     * @throws IllegalArgumentException if there is no players online.
     */
     ...
     *
     * @return A {@link CompletableFuture} that, when completed, will return a
     * list of server name strings, as defined in BungeeCord's
     * config.yml.
     * @throws IllegalArgumentException if there is no players online.
     */
    ...
    /**
     * Send a custom plugin message to said server. This is one of the most
     * useful channels ever. <b>Remember, the sending and receiving server(s)
     * need to have a player online.</b>
     *
     * @param server      the name of the server to send to, ALL to send to every server
     *                    (except the one sending the plugin message), or ONLINE to send
     *                    to every server that's online (except the one sending the
     *                    plugin message).
    ...
    private Player getFirstPlayer() {
         ...
         */
        Player firstPlayer = getFirstPlayer0(Utils.getOnlinePlayers());
